...
```console
$ rss-proxy backup rss-proxy-20260915.db
バックアップしました: rss-proxy-20260915.db
```

稼働中でも取れる。DB ファイルを直接コピーする方法は、WAL を使っているため直近の書き込みが欠けることがある。

戻すときはサーバーを停止して DB ファイルを置き換える。出力は通常の SQLite ファイルなので、専用のコマンドは要らない。
...
気象庁のフィードは entry 自体に市区町村の情報を持たない。リンク先の XML を取って初めて分かる。

**購読するフィードは長期フィード (`https://www.data.jma.go.jp/developer/xml/feed/extra_l.xml`) を勧める。** 高頻度フィード (`extra.xml`) でも動くが、残っている期間が短いため、継続中の警報の発令時刻 (後述) を遡れないことが多い。
...
```
title: 兵庫県気象警報・注意報
link:  https://www.jma.go.jp/bosai/warning/#area_type=offices&area_code=280000

2026-09-09 04:58 時点
・神戸市東灘区: 大雨警報(9/8 21:30〜), 雷注意報(02:10〜), 強風注意報
・神戸市灘区: 【新】大雨警報, 雷注意報(02:10〜), 強風注意報
・神戸市兵庫区: 大雨警報(9/8 21:30〜), 雷注意報(02:10〜), 強風注意報
```

- **リンクは気象庁の警報ページに差し替わる。** フィードの link は XML を指しており、リーダーから開いても読めない
- **先頭に発表時刻が入る。** いつ時点の情報かが分かる
- **地域は `・` で始まる箇条書き。** 行頭が揃い、改行を潰す読み手でも切れ目が分かる

**今回新しく発表されたものには印が付く。** 前回の受信内容を保存しているのではなく、気象庁の XML が種別ごとに「発表」「継続」を区別しているのでそれを使っている。

**継続中のものには、分かれば発令時刻が付く。** `(02:10〜)` は発表と同じ日、`(9/8 21:30〜)` はそれより前の日。XML には発令時刻が無いため、フィードに残っている過去の発表を遡って「継続」でなくなった回を探している。フィードの範囲内で見つからなければ何も付けない (上の例の強風注意報)。過去の発表も取得するので、購読し始めの数回は取得件数の上限 (1 回 10 件) に達し、発令時刻が揃うまで少し掛かる。

印の書式は読み手に合わせて変えられる。

| 設定 | 表示 | 向き |
|------|------|------|
| `{"new_prefix":"【新】"}` (既定) | `【新】大雨警報` | どのリーダーでも確実に見える |
| `{"new_prefix":"*","new_suffix":"*"}` | `*大雨警報*` | Slack の太字 |
| `{"new_prefix":"**","new_suffix":"**"}` | `**大雨警報**` | Markdown を解釈するリーダー |
| `{"new_prefix":"<strong>","new_suffix":"</strong>"}` | 太字 | HTML を解釈するリーダー |
| `{"new_prefix":"🆕 "}` | `🆕 大雨警報` | 絵文字 |
| `{"new_prefix":"","new_suffix":""}` | `大雨警報` | 印を付けない |

Slack の `/feed` は HTML タグを剥がすため、`<strong>` は効かない可能性が高い。既定の `【新】` か、Slack 独自の `*` を試すのが確実。

URL に府県コードが入っているため、取りに行く前に対象を判別できる。取得した XML は URL 単位で保存しており、一度取った発表は取り直さない。
