...
変換するのは U+FF01〜U+FF5E (全角 ASCII) だけ。カギ括弧 (`「」『』`)、句読点 (`、。`)、なかてん (`・`)、波ダッシュ (`〜`) はいずれもこの範囲の外にあるため、何もしなくてもそのまま残る。範囲内にある全角チルダ (`～` U+FF5E) だけは日本語で区間を表す記号として使われるので明示的に除く。

全角コロン (`：`) や全角括弧 (`（）`) は変換対象に含まれる。除外したい記号が増えたらこの一覧に足す。
