...
一方、管理画面に出す時刻 (最終取得) は運用者のローカルタイムで表示する。UTC のまま出すと日本時間との差を頭の中で足すことになる。どの地域で読んでも誤解しないよう、オフセットを必ず添える (`2026-09-07 14:33 +09:00`)。
...
1. フィード一覧 (`/`) — 名前、タイトル、間隔、最終取得、状態、配信 URL。登録フォーム
