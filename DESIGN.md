...
取得は scheduler が行う。URL 単位でキャッシュし (`documents` テーブル)、1 回の巡回で取りに行く数に上限を置く (10 件)。`wants` の並びを取得の優先順として扱い、上限で打ち切られても先頭から取る。気象庁の XML のように URL が発表ごとに変わるものは、一度取れば取り直す必要がない。
...
rss-proxy opml export --base-url URL # 配信 URL の一覧を OPML で書き出す
```
...
出す対象は `feeds` (slug / label / url / interval_secs) と、フィードごとおよび共通の Processor 連鎖。連鎖は順序が意味を持つのでそのまま保つ。`params` は DB では文字列だが、読めるように展開して埋める。読めない値だった場合も文字列のまま出す (書き出しは設定を残すためのもので、捨てられるのは中身を見た人だけ)。

フィードは登録順ではなく識別子順に並べる。別の環境で組み直しても同じ並びになり、版管理に載せたときの差分が読みやすい。
...
配信 URL の一覧を OPML 2.0 で書き出す。識別子は省略すると乱数から作られるため、RSS リーダーに登録するには管理画面から 1 つずつ写すことになる。OPML ならリーダー側に一括で取り込める。

出口は 2 つ。生成は共通の関数 (`opml::render`) に置く。

- `GET /opml` — 管理画面からリンク 1 本で取れる。起点はリクエストから組み立てる。スキームは `X-Forwarded-Proto` があればそれを使い、なければ `http` (リバースプロキシの背後に置く構成のため)
- `rss-proxy opml export --base-url URL` — サーバーを起動していなくても取れる。CLI 側にはホスト名を知る手段がないので `--base-url` は必須

`GET /opml` は管理画面と同じ認証ガードの下に置く。配信 URL の一覧がまとめて出るため、`/feeds/*` と同じ扱いにはしない。

`outline` の `text` / `title` は表示名、なければ上流のタイトル、それもなければ識別子。空欄ではリーダーの一覧で見分けられない。属性に入れる値は XML エスケープする。表示名も上流のタイトルも外部の入力であり、引用符を含んでいれば属性を閉じて壊せる。起点も同じく外部から来る (CLI の引数、HTTP の `Host` ヘッダ) ため、`xmlUrl` は組み立ててから 1 回通す。

フィードの並びは識別子順 (`config export` と揃える)。

階層 (`outline` の入れ子) は使わない。フィードを分類する仕組みが本体にないため、平坦に並べる。

OPML の取り込みは見送る。実体は `feed add` の一括実行で、初期移行の 1 回きりしか使わない。
...
DB スキーマのマイグレーションは起動時に自動適用する。適用前に DB のスナップショットを取り、`<DB ファイル名>.bak-v<適用前の段数>` として同じディレクトリに残す。マイグレーションには列の rename や drop が含まれ、途中で失敗すると元に戻せないため。

取得には `VACUUM INTO` を使う。WAL を有効にしているため DB ファイルを単にコピーしても直近の書き込みが `-wal` 側に残り、一貫したコピーにならない。`VACUUM INTO` は WAL の内容を含んだスナップショットを単一ファイルに書き出す。

進める段がないときは取らない。起動のたびにファイルが増えないようにするため。フィードが 1 件もない DB も取らない (新規インストールには残す状態がない)。同名のファイルが既にあるときも取らない (前回の移行前の状態を壊さない)。インメモリ DB には保存先がないので対象外。

スナップショットが取れなかった場合は移行に進まず起動を止める。戻せない状態のまま取り返しのつかない操作を適用するより、起動しないほうが安全。
