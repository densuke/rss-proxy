...
async fn refresh_stores_processed_output() {
    ...
    let feed = store.feed_by_name("news").unwrap().unwrap();
    ...
    let after = store.feed_by_name("news").unwrap().unwrap();
...
async fn not_modified_skips_processing_but_reschedules() {
    ...
    assert!(store.feed_by_name("news").unwrap().unwrap().next_fetch_at >= next_before);
}
